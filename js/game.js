...
function updateRecordDisplay() {
    let record = localStorage.getItem(`tampidoku_record_${SIZE}`);
    document.getElementById('record-display').innerText = record ? formatTime(parseInt(record)) : "--:--";
}

function initBoard() {
    let newLevel = generateRandomLevel(SIZE);
...
if (typeof window !== 'undefined') {
    ...
    window.addEventListener('pointerup', (e) => {
        ...
        if (!isDrawingCrosses && !isRightMouseButton && !pointerMoved) {
            const releaseCell = getCellFromPoint(e.clientX, e.clientY);
            if (releaseCell === pressStartCell && pressStartCell) {
                const { r, c } = getCellCoordinates(pressStartCell);
                tryPlaceMarker(r, c, pressStartCell);
            }
        }
...
function resetGame() {
    ...
    document.getElementById('level-display').innerText = `${SIZE}x${SIZE}`;
...
function triggerError(cell, r, c) {
    ...
    if(lives <= 0) {
        ...
        msg.innerText = "¡Juego Terminado! Los vecinos se pelearon.";
        msg.style.color = "red";
        setTimeout(() => { SIZE = 6; resetGame(); }, 3000);
    }
}
...
function checkWin() {
    if (markersPlaced === SIZE) {
        ...
        let currentRecord = localStorage.getItem(`tampidoku_record_${SIZE}`);
        let isNewRecord = false;
        
        if (!currentRecord || secondsElapsed < parseInt(currentRecord)) {
            localStorage.setItem(`tampidoku_record_${SIZE}`, secondsElapsed);
            isNewRecord = true;
        }
